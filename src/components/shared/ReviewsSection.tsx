    ...
    async function fetchData() {
      try {
        const [reviewsRes, statsRes] = await Promise.all([
          fetch('/api/reviews?visible=true'),
      ...
      <section className="py-16 lg:py-24 bg-cream">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="animate-pulse">
            <div className="h-10 w-64 bg-sage/20 rounded mx-auto mb-12" />
          ...
          {stats && stats.count > 0 && (
            <div className="flex items-center justify-center gap-3">
              <div className="flex items-center gap-1">
                <span className="text-2xl font-bold text-gold">{stats.average}</span>
                <svg className="w-6 h-6 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              </div>
              <span className="text-sage">|</span>
              <span className="text-sage">{stats.count} avis vérifiés</span>
            </div>
          )}
        </div>
              ...
              {review.photos.length > 0 && (
                <div className="flex gap-2 mt-4">
                  ...
                  {review.photos.length > 3 && (
                    <div className="w-16 h-16 bg-sage/10 rounded-lg flex items-center justify-center text-sage text-sm font-medium">
                      +{review.photos.length - 3}
                    </div>
                  )}
                </div>
